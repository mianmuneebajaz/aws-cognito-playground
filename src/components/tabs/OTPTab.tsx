...
export const OTPTab: React.FC<OTPTabProps> = ({ activeConfigName }) => {
  const [mfaStep, setMfaStep] = useState<"setup" | "verify" | "authenticate">(
    "setup"
  );
  ...
  useEffect(() => {
    const generateQRCode = async () => {
      if (setupData.qrCodeUri) {
        try {
          console.log("Generating QR code for URI:", setupData.qrCodeUri);
          console.log("URI type:", typeof setupData.qrCodeUri);
          console.log("URI length:", setupData.qrCodeUri.length);
          ...
          console.error("URI that failed:", setupData.qrCodeUri);

          // Set an error state so we can show a message to the user
          setResult({
            type: "error",
            message:
  ...
  const handleAuthenticateMFA = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "MFA authentication failed";
      setResult({ type: "error", message });
    } finally {
      setIsLoading(false);
    }
  ...
  const renderAuthenticateStep = () => (
    <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
      ...
      <p className="text-slate-600 mb-4">
        During sign-in, you'll be prompted to enter a verification code from
        your authenticator app. You can test the authentication process below:
      </p>
