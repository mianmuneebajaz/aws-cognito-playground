...
import { saveConfigurations, loadConfigurations, clearAllData } from '../../utils/storage';
...
export const ConfigurationTab: React.FC<ConfigurationTabProps> = ({ 
  ...
  const handleSave = () => {
    if (!currentConfig.name || !currentConfig.region || !currentConfig.userPoolId || !currentConfig.userPoolWebClientId) {
      showMessage('error', 'Please fill in all required fields');
      return;
    }

    const newConfig = currentConfig as CognitoConfig;
    const updatedConfigs = [...configs.filter(c => c.name !== newConfig.name), newConfig];
  ...
  const handleLoad = async (config: CognitoConfig) => {
    try {
      ...
      showMessage('error', `Failed to load configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  const handleReset = async () => {
    try {
      ...
      showMessage('error', `Failed to reset configuration: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  const handleClearAll = async () => {
    try {
      ...
      showMessage('error', `Failed to clear all data: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <h2 className="text-xl font-semibold text-slate-900 mb-4">AWS Cognito Configuration</h2>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setCurrentConfig({ ...currentConfig, region: e.target.value })}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCurrentConfig({ ...currentConfig, userPoolId: e.target.value })}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCurrentConfig({ ...currentConfig, userPoolWebClientId: e.target.value })}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setCurrentConfig({ ...currentConfig, identityPoolId: e.target.value })}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setCurrentConfig({ 
                ...currentConfig, 
                authenticationFlowType: e.target.value as CognitoConfig['authenticationFlowType']
              })}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
              <option value="USER_SRP_AUTH">USER_SRP_AUTH</option>
              <option value="USER_PASSWORD_AUTH">USER_PASSWORD_AUTH</option>
              <option value="ALLOW_USER_PASSWORD_AUTH">ALLOW_USER_PASSWORD_AUTH</option>
            </select>
          </div>
        </div>
        ...
        <div className="flex items-start space-x-2">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 mt-1">
              Configurations are stored in browser local storage. This data is not encrypted and may be accessible to browser extensions or scripts. Do not store production credentials.
            </p>
            <p className="text-sm text-amber-700 mt-2">
              <strong>Note:</strong> Switching configurations will automatically sign out any currently authenticated user to prevent authentication conflicts.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-500 bg-opacity-75">
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            ...
            <p className="text-slate-600 mb-4">
              This will permanently delete all saved configurations and sign out any authenticated user. This action cannot be undone.
            </p>
